...
export const POST: RequestHandler = async (event) => {
  ...
  if (authError) {
    console.error('[privacy/follow-deny] auth error', authError);
  ...
  const { error } = await supabase.rpc('deny_follow', { requester: requesterId });
  if (error) {
    console.error('[privacy/follow-deny] rpc failed', error);
