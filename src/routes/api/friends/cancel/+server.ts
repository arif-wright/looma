...
type CancelPayload = { requestId?: string };
...
export const POST: RequestHandler = async (event) => {
  ...
  if (error) {
    ...
    return json(
      { error: status === 404 ? 'request_not_found' : 'bad_request', message: status === 404 ? 'Friend request not found.' : 'Unable to cancel request.' },
