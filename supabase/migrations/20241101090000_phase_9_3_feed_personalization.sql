-- Phase 9.3 — Personalized feed view and navigation groundwork

-- Ensure posts table supports engagement weighting and deep links (idempotent).
...
-- Promote last_context to structured JSON storage.
do $$
begin
  if exists (
    select 1
...
-- Lightweight follows table for social graph (idempotent).
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'follows_select_visibility'
  ) then
...
-- Supporting indexes for feed ordering (idempotent).
...
-- Personalized feed view with weighting for follows, recency, and engagement.
