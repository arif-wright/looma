...
Directional selection is not billboard orientation. Orthographic selection uses the camera's backward X/Z viewing vector, so player-follow translation cannot change a stationary prop's view unless camera yaw actually changes. Broadleaf presentation copies the finalized camera quaternion so the large plane stays parallel to the screen; its geometry is translated around the declared image-space ground anchor, leaving the mesh origin planted at the server-mapped X/Z point. The camera transform is never used as tree gameplay-facing. Environment LOD uses horizontal X/Z distance from the orthographic view focus, not the physical orbiting camera, so camera height and orbit radius cannot disable an on-screen animation.
