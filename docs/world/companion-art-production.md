...
Use a 256×256 transparent source cell for Muse, fixed orthographic camera when an approved current model is used, identical camera position/focal length, consistent neutral lighting, no baked background or ground plane, no frame-to-frame camera movement, and consistent silhouette scale. Supply every approved frame without downsampling and declare each authored direction's actual count and timing; direction counts may differ. The current Muse source supplies native N/NE/E/SE/S artwork for idle and walk; NW/W/SW are explicitly packed as horizontal mirrors of NE/E/SE. Foot contact must remain aligned to the declared feet point even when the body bobs. The older sheet set with a missing walk NE direction is Echo artwork and is maintained under Echo's production directory and manifest.
