...
Phase 8C.5 replaces the Three primitives with approved production terrain and camera-facing environment cards. Static and animated instances share textures and geometry; animated instances own only frame/opacity uniforms. A deterministic instance hash offsets phase and playback rate without synchronization or persistence. Terrain remains world-aligned geometry, preserves source proportions through repeat wrapping, and uses an irregular path strip independent of traversal. Source-to-runtime copies and provenance are defined in `environment-production-asset-pipeline.md`.
