...
1. **Create a Services ID** in the Apple Developer portal (`Certificates, Identifiers & Profiles` → `Identifiers`). Use a reverse-DNS identifier (for example `com.kinforge.looma.auth`). Enable `Sign in with Apple` and add the callback URL `https://memvoya.com/auth/callback` as well as `http://localhost:5173/auth/callback` for local development.
